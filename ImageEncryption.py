...
def chosen_plaintext_attack_test(password: str, mu: float = 3.99, dna_operation: str = "add", output_dir: Path = None):
    ...
    print(f"\n{'='*80}")
    print(f"🎯 CHOSEN PLAINTEXT ATTACK (CPA) ANALYSIS") 
    print(f"{'='*80}")
    ...
    print(f"   🎨 Creating 2x2 histogram plot...")
    ...
    axes[0, 0].text(0.02, 0.98, f'Size: 256×256\nValue: 255\nEntropy: 0.000 bits\nStd Dev: 0.000', 
    ...
    axes[1, 0].text(0.02, 0.98, f'Size: 256×256\nValue: 0\nEntropy: 0.000 bits\nStd Dev: 0.000', 
    ...
    print(f"   ✅ CPA Histogram plot saved: {histogram_file}")
    
    print(f"   🎨 Creating additional comparison plot...")
...
def process_single_image(image_path: Path, output_dir: Path, password: str = "medical123", mu: float = 3.99, dna_operation: str = "add"):
    ...
    try:
        ...
        if HAVE_PYDICOM:
            try:
                save_encrypted_dicom(cipher_img, image_path, image_folder, aux)
            except Exception:
                pass

        if image_path.suffix.lower() == '.dcm' and HAVE_PYDICOM:
            try:
                save_encrypted_dicom(cipher_img, image_path, image_folder, aux)
            except Exception:
                pass
...
def chosen_plaintext_attack_test(password: str, mu: float = 3.99, dna_operation: str = "add", output_dir: Path = None):
    ...
    print(f"   ✅ CPA Comparison plot saved: {comparison_file}")
    
    print(f"\n✅ CPA Analysis Complete! Results saved to: {output_dir}")
    print(f"   📊 Generated Files:")
    print(f"     • cpa_results.csv - Analysis results")
    print(f"     • CPA_Histogram_Analysis.png - Main 2x2 histogram plot")
    print(f"     • CPA_Before_After_Comparison.png - Before/after comparison")
    
    return cpa_results

def generate_cpa_histogram_analysis(output_dir: Path, password: str, mu: float = 3.99, dna_operation: str = "add"):
   
    print(f"📊 Generating CPA Histogram Analysis...")
    ...
    print(f"   ✅ CPA Histogram Analysis saved: {output_dir}/CPA_Histogram_Analysis.png")
...
=====================
1. ✅ Complete Pattern Concealment: Both uniform patterns produce statistically random outputs
2. ✅ Cross-Pattern Independence: Different patterns produce uncorrelated encrypted results
3. ✅ Maximum Entropy Achievement: Both encrypted images approach maximum entropy (8.0 bits)
4. ✅ Uniform Distribution: Encrypted histograms show near-uniform pixel value distribution
...
against advanced cryptanalytic attacks.
    ...
    print(f"   📋 Detailed CPA statistics saved: {output_dir}/CPA_Detailed_Statistics.csv")
    print(f"   📄 CPA analysis report saved: {output_dir}/CPA_Analysis_Report.txt")

def create_cpa_histograms_simple(output_dir: Path, password: str, mu: float = 3.99, dna_operation: str = "add"):
 
    print(f"\n📊 Creating CPA Histogram Analysis...")
    ...
    try:
        ...
        print(f"   ✅ CPA Histogram saved: {output_file}")
        return True
        
    except Exception as e:
        print(f"   ❌ Error creating CPA histograms: {str(e)}")
        import traceback
        traceback.print_exc()
        return False
